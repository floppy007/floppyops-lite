...
## v1.3.1 (2026-05-31) — Security Audit Follow-up

Full-application security audit (command injection, config injection, XSS, sudo least-privilege) with every finding fixed and re-verified.

### Security
- **WireGuard live peer changes no longer run through a shell.** `wg-add-peer` and `wg-remove-peer` built `shell_exec("sudo wg set $iface peer $peerPubKey allowed-ips $peerAllowedIps …")` directly from POST fields. An authenticated user (including a non-root member of the `sudo`/`wheel` group, who can log in but should not have full root) could inject `; …` / `$(…)` through `public_key`/`allowed_ips` and execute arbitrary commands as root. These now run via `proc_open` with an argv array (no shell), and the public key (base64, 44 chars) and AllowedIPs (CIDR list) are strictly validated first.
- **WireGuard config writes can no longer smuggle a root `PostUp`.** `wg-create`, `wg-add-peer` and `wg-update-peer` wrote `Address`, keys, `Endpoint`, `AllowedIPs`, `PresharedKey` and the client meta-comment fields into the `.conf` after only `trim()`. A newline in any field injected a real `PostUp = …` line that `wg-quick` runs as root on the next start — bypassing the v1.3.0 PostUp allowlist, which only ran on the dedicated hook fields. All of these fields are now type-validated (key charset, CIDR, `host:port`) and rejected if they contain CR/LF; peer names strip line breaks.
- **nginx reverse-proxy editing can no longer inject arbitrary nginx directives.** `nginx-update` wrote the `ip`/`port`/`domains` POST fields into the site config with no validation (unlike `nginx-add`), so a crafted value could inject directives such as `alias /;` (arbitrary file read of `/etc/shadow`, certbot keys) or an internal `proxy_pass`. The fields are now validated (IP/host, port range, anchored domain regex) before the config is written. Input-validation regexes in `nginx-add` and `nginx-renew` were re-anchored with `\A…\z` so a trailing newline can't slip past.
- **Client-side output is now HTML-escaped everywhere.** Server data that an attacker can influence — fail2ban-banned IPs and log lines, WireGuard peer names/endpoints/keys, nginx server-names and certificate fields, VM/CT names, firewall rule fields, port-scan process names, the request `Host` header — was interpolated raw into `innerHTML` and inline `onclick` handlers across every JS module, allowing stored/DOM XSS. A shared `escapeHtml()` (plus `escapeJsArg()` for inline handlers) is now applied to all of it; `toast()` renders its message as text, not markup.
- **sudo allowlist tightened to least privilege.** The deployed `/etc/sudoers.d/server-admin` had accreted catch-all rules (`cp *`, `chmod *`, `chown *`, `iptables *`, `pvesh *`, `cat /etc/pve/*`), each a trivial `www-data` → root escape (`chmod u+s /bin/bash`, `cp` into `/etc/sudoers.d`, `pvesh create /access/users`, `iptables --modprobe`, reading `/etc/pve/priv/` cluster secrets). These are replaced by rules scoped to the exact commands and paths the panel uses; `pvesh` write verbs are limited to `/nodes/` and `/cluster/firewall/` (PVE admin-user creation is no longer reachable), and `cat`/`cp` of PVE configs are scoped to the guest-config directories. `certbot` and `pct exec` remain broad by necessity (their arguments are inherently arbitrary) and are reachable only by the authenticated, CSRF-protected admin.
- **The sudo ruleset now has a single source and `update.sh` repairs drift instead of growing it.** The ruleset lives in `helpers/sudoers-rules.sh`; both `setup.sh` and `update.sh` generate the identical, `visudo`-validated file. Previously `update.sh` only *appended* missing lines (`ensure_sudoers_line`), so a host that already had the broad catch-alls kept them after an update — the exact mechanism that let the allowlist drift wide in the first place. `update.sh` now fully regenerates the file atomically, so running it on an old install actively *removes* the dangerous catch-alls and converges on the hardened set.

### Improved
- **nginx site creation now shows live, step-by-step progress.** Adding a reverse-proxy site opens a progress modal that reports each stage (config written, site enabled, `nginx -t`, reload, certbot/SSL, delivery verification) with its own output, instead of a single spinner. Bilingual (DE/EN).
- **Deep-linking via URL hash.** Opening `/#network` (or `#zfs`, `#system`, the legacy `#wireguard`/`#firewall` names, …) jumps straight to that tab and sub-tab on load and triggers its data load, so bookmarks and shared links land where expected.
- **Favicon is now served from `/favicon.png`** instead of an inline base64 data URI, shrinking the HTML and letting the browser cache it.
- **JS assets are cache-busted by file modification time** (`?v=<version>-<mtime>`), so client updates take effect immediately after an update without a manual version bump.

## v1.3.0 (2026-05-11) — Security Release

### Security (breaking)
- **Panel access now requires admin-level authorization, not just a valid login.** Previously every PVE user with a valid ticket and every successful PAM authentication granted full panel control — including users with read-only PVE roles. The panel now mirrors PVE's own authorization model: after authentication it queries `/access/permissions` and requires `Sys.Modify` + `Sys.PowerMgmt` on `/` (the rights the `Administrator` role grants). PAM users must be root or a member of the `sudo`/`wheel` group. Operators who relied on logging in with a non-admin PVE user will be denied with a clear error message and need to grant their account the appropriate role (or use root).
- **WireGuard `PostUp` / `PostDown` directives are now validated against a strict allowlist.** `wg-quick` executes these lines as root when a tunnel starts, so an authenticated user with import/edit access could previously smuggle arbitrary root commands through the config (e.g. via `wg-import`, the config editor, or the new-tunnel form). The allowlist covers exactly the patterns the in-UI wizard produces (`echo {0,1} > /proc/sys/net/...`, `iptables`/`ip6tables` NAT/FORWARD rules, `sysctl -w net.*`, `ip route add/del`, `resolvconf`). `wg-create` and `wg-save` reject configs with disallowed commands; `wg-import` strips them with a warning so migration flows still work. If you had a non-standard PostUp in a manually-managed tunnel, the panel will refuse to save edits until you either bring it into the allowlist or set the line outside the panel.
- **WireGuard public-key derivation no longer goes through a shell.** The old `echo '$priv' | wg pubkey` pattern was reachable from imported config content; a private-key field containing a single quote let the imported config break out and execute commands as `www-data` (which trivially escalates via the sudo allowlist). `wg pubkey` now runs through `proc_open` with the key written to stdin, and inputs are pre-validated as base64.

### Hardened
- **Session cookies are now HttpOnly + Secure + SameSite=Strict, and the session id is regenerated on login.** The previous defaults left the cookie readable by JavaScript and accepted on cross-site navigations; combined with the absence of `session_regenerate_id()` this allowed session fixation (pre-set the visitor's `PHPSESSID`, wait for them to log in, inherit their authenticated session). Secure is gated on `$_SERVER['HTTPS']` so plain-HTTP installs aren't locked out, but operators behind TLS get the full hardening automatically.
- **nginx security headers refreshed and now applied on update, not only on first install.** The previous `if [[ ! -f ... ]]` gate meant existing installations never received header updates. `update.sh` now rewrites `/etc/nginx/conf.d/security-headers.conf` every run. `X-Frame-Options` is tightened from `SAMEORIGIN` to `DENY` (the panel doesn't need to frame itself), and a Content-Security-Policy is added with `frame-ancestors 'none'`, `default-src 'self'`, scripts pinned to `'self'` + jsdelivr (Chart.js), and `connect-src 'self'` so injected scripts can't exfiltrate to a third-party origin. The deprecated `X-XSS-Protection` header is dropped.
- **PVE API auth call verifies TLS against PVE's own root CA.** The previous `verify_peer => false` only happened to be safe because the URL was `127.0.0.1` — any future change to the API hostname would have silently accepted a forged certificate. Verification now uses `/etc/pve/pve-root-ca.pem` as trust anchor and connects to the node hostname so SNI and CN line up. If the CA file isn't available, the panel falls back to the old unverified behavior so a non-standard PVE host still works.
- **Login form is now CSRF-protected via a double-submit cookie.** Before the session exists there's no token to compare against, so the panel issues a random `_login_csrf` cookie + matching hidden form field on every login render and requires both to match on submit. This blocks the login-CSRF variant where an attacker silently logs the victim into an attacker-controlled account.
